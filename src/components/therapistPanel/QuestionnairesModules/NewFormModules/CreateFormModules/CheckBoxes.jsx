...
const CheckBoxes = ({handleQuestionChange,id,answerConfig}) => {
  ...
  const handleInputChange = (index, event) => {
    setOptions({...options,[index]:event.target.value});
  ...
  return (
    <>
      <div className="col gap-4 w-full">
        ...
        <CloseIcon onClick={()=>setInputs(inputs.filter((item)=>item.value!==input.value))} className="!w-4 !h-4" />
      </div>
            </div>
          ))}
        </span>
        <span className="flex gap-2">
          ...
          <span onClick={handleAddInput} className=" cursor-pointer text-sm flex gap-2">
            {" "}
            <h5 className="dark:text-neutral-300">Add option</h5> or{" "}
